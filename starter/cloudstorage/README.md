...
- [x] You should inform the user with a success message after each operation (create/edit/delete). At the moment, there's no message for the user that says something like "The Credential has been added to the list", etc.
- [x] Good job preventing users to create already used usernames. However, it's still possible to upload two files with the same name as you can see below.
...
- As for the second error encountered below, it said "element click intercepted", which didn't occur to me with many tries. In my opinion it's caused by clicking other element or window while the tests are running.
- Could you please re-run the test and see if the errors show again?
![](Screenshot__2095_.png)
